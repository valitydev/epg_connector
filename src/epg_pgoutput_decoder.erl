...
decode_tuple_columns(<<Kind:8, Rest/binary>>, NumColumns, Acc) when
    Kind =:= ?TUPLE_VALUE_KIND_NULL; Kind =:= ?TUPLE_VALUE_KIND_UNCHANGED
->
    decode_tuple_columns(
        Rest,
        NumColumns - 1,
        [
            #column_value{
                kind = decode_tuple_value_kind(Kind),
                value = decode_tuple_value_kind(Kind)
            }
            | Acc
        ]
    );
